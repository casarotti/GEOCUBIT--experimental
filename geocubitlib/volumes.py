...
def surfaces(filename=None,):
    ...
    if cfg.volume_type == 'layercake_volume_ascii_regulargrid_regularmap':
        ...
        layercake_volume_fromacis_mpiregularmap(
            filename=filename, onlysurface=True)
    elif cfg.volume_type == sandwich:
        layercake_volume_ascii_regulargrid_mpiregularmap(
...
def layercake_volume_fromacis_mpiregularmap(filename=None):
    ...
    def translate2zero():
        ...
        xmin = box[0]
        ymin = box[3]
        cubit.cmd('move surface all x ' +
                  str(-1 * xmin) + ' y ' + str(-1 * ymin))
        return xmin, ymin
    ...
    # length of y slide for chunk
    ylength = (cfg.ymax - cfg.ymin) / float(cfg.cpuy)
    ...
    for inz in range(cfg.nz - 2, -2, -1):
        if cfg.bottomflat and inz == -1:
            command = "create planar surface with plane zplane offset " + \
                str(cfg.depth_bottom)
            cubit.cmd(command)
        else:
